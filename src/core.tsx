  ...
  forwardRef<PrismEditor, EditorProps>((props, ref) => {
    ...
    const updateSelection = (force?: true) => {
      if (handleSelectionChange || force) {
        ...
        const newLine =
          editor.lines![
            (activeLineNumber = numLines(
              value,
              0,
              selection[selection[2] < 'f' ? 0 : 1]
            ))
          ];
    ...
    const update = () => {
      ...
      if (start == end1 && start == end2)
        ...
        for (i = insertStart + 1; i < lineCount; )
          lines[++i].setAttribute('data-line', i as any);
    ...
    const textareaRef = useStableRef((el: HTMLTextAreaElement | null) => {
      if (el && !textarea) {
        ...
        addTextareaListener(editor, 'keydown', (e) => {
          keyCommandMap[e.key]?.(e, getInputSelection(), value) &&
            preventDefault(e);
        });
        ...
        <div
          ...
          ref={useStableRef((el: HTMLDivElement | null) => {
            if (el) {
              editor.wrapper = el;
              editor.lines = lines =
                el.children as HTMLCollectionOf<HTMLDivElement>;
            }
          })}
        >
